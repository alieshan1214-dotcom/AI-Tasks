...
# Setting up a clock to regulate smooth frame rates instead of heavy waiting delays
...
# 3rd parameter is font colour and 4th parameter is Font background
...
# infinite loop
while not done:
    # Handle window events smoothly so it doesn't crash on close
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            done = True

    # Check loop boundaries for resetting animation loops
    if i >= 820:
        i = 0
        c += 1
    ...
    if c % 6 == 0:
        ...
        i += 4  # Reduced from 80 for smoother motion
    ...
    # Cap at 60 frames per second for smooth progression
    clock.tick(60)
